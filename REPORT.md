...
either strategy to bridge `λ > 4/k` to `M_k > 4`. The algebraic
prerequisite for any future closure of Lemma 8.3 — symmetry of the
Gram matrices `M1`, `M2` — is now formalised in `MaynardBridge.v`
(see §3.8); the analytic content of Lemma 8.3 itself remains
paper-side, by design of this project's scope.
...
**27 `.v` files**, totaling **~19 500 lines**, of which ~7 500 are
autogenerated certificate data (`Witness.v` + `WitnessChain.v`). The
hand-written proof script is ~12 000 lines.
...
`MaynardVerify.v`, `MaynardBridge.v` close the trust loop on the 42x42
input matrices. They sit downstream of `Witness.v` and `CharPoly.v`
but are leaves in the dependency DAG — `Cert.v` imports
`MaynardVerify` (for the `all_match_*` bool facts inside
`maynard_M105_certified`) but the others, including `MaynardBridge`,
are not on the headline path.
...
**`MaynardBridge.v`** (~290 lines). Algebraic groundwork for a future
formalisation of Maynard's Lemma 8.3 (the Rayleigh-quotient identity
`M_k = k · λ_max(M1⁻¹·M2)`). It proves:

- Rat-level: `M1_entry`, `M2_entry` are symmetric in
  `(bi, ci) ↔ (bj, cj)`. Maynard's closed forms are visibly
  symmetric; the lemmas formalise that symmetry inside Rocq.
- Z-level: `M1_int`, `M2_int` (the matrices shipped in `Witness.v`)
  are symmetric, by a single `vm_compute` on a 42×42 boolean grid.
- The rat-level matrices `mat_int_to_rat M{1,2}_int D_M{1,2} 42`
  are symmetric, deduced from the Z-level fact via
  `mat_int_to_rat`'s defining equation.
- The standard MathComp idiom `trmx M = M` for these matrices.

`MaynardBridge.v` is a leaf in the dependency DAG (it is *not*
imported by `Cert.v`) and is not on the headline path. It addresses
audit finding M-2 partially: it formalises the algebraic
prerequisites that any future closure of Lemma 8.3 / spectrum-reality
arguments would need (positive-definiteness of `M1` over ℚ and the
spectral theorem for symmetric rational matrices remain mathematical
work outside the current MathComp library; the file documents these
gaps explicitly). The headline `M_{105} > 4` claim does not depend
on this file — its scope here is the Sturm-chain certificate, which
is a different bridge from `λ > 4/k` to `M_k > 4` than the
...
- **Total Rocq proof size.** 27 `.v` files, ~19 500 lines; ~7 500
  autogenerated certificate data; ~12 000 hand-written.
