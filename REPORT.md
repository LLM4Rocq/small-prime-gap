...
is `Qed` in `theories/S1/CertQuad.v`. A single
`Print Assumptions maynard_M105_certified_alt` covers (a) the
closed-form match between the FLINT-shipped `M1_int / M2_int` and
the readable paper-form spec `MaynardSpec.M{1,2}_spec_ij` (which
transcribes Maynard's Lemma 8.2 character-for-character), and (b)
the strict Rayleigh-quotient bound at the shipped 42-entry rational
witness. The companion document `AUDITOR_CHECKLIST.md` enumerates
the load-bearing lemmas in audit order.

The proof has one current `Admitted`: the rat-level Rayleigh
inequality `CertQuad.rayleigh_lt_main` (a thin algebraic shim that
clears denominators against the rat-level bigop ⇄ Z bridge
`quad_spec_eq_Z` and lifts the integer Rayleigh inequality
`rayleigh_witness_holds` via `Z2rat_lt`). The integer Rayleigh
inequality itself (`rayleigh_witness_holds`) and its positivity
companion (`rayleigh_witness_M1_positive`) are both `Qed` and
*Closed under the global context* — pure `Z`-arithmetic by
`vm_compute` reflexivity. The block on `rayleigh_lt_main` is purely
operational (kernel proof-term verification OOMs under the 16 GB host
cgroup limit when the per-cell `field` identity is unfolded inside
the 42×42 nested binders); the structural proof is documented in the
header of `CertQuad.v` and the per-cell algebraic identity is itself
Qed-sealed as `quad_cell_identity`.
...
emits `Witness_Quad.v` containing a 42-entry rational vector
...
The FLINT layer (§2.1) is the **candidate generator**: it computes
matrix entries and a Rayleigh-quotient witness, ships them as Rocq
source. The Rocq layer (§2.2) is the **verification**: it consumes
that certificate as untrusted input data and kernel-checks every
matrix entry against an independent Rocq-side closed form
(`MaynardVerify` against `MaynardSpec`) and the Rayleigh inequality
at the shipped witness against pure-Z arithmetic (`CertQuad`).
Only the Rocq layer's Qeds are in the trust base; the FLINT layer
is auxiliary.
...
6. Emit `theories/S1/Witness.v` (matrix entries and basis) and
   `theories/S1/Witness_Quad.v` (42-entry rational witness).
...
**9 547 lines** under `theories/S1/`, of which `Witness.v` alone
...
denominators `D_M1, D_M2`. On this branch the additional fields
shipped by the FLINT pipeline (the integer matrix `A_int`, the
scalars `D_A` and `D_q`, and the cleared char-poly coefficient
lists) are unused: no downstream file imports them. Large integers
are written as `bigZ` to keep parser time manageable.

**`Witness_Quad.v`** (autogenerated). The Rayleigh-quotient
...
operations. Retained on this branch as a dependency of `CharPoly.v`
(below); the polynomial-side operations are not consumed by the
...
**`CharPoly.v`**. On this branch, this file contributes only the
Z to rat / Z to int bridging definitions consumed downstream:
...
are not transitively reached from `CertQuad.v`'s `Require Import`s.
...
**`MaynardFactQ.v`** (~30 lines). Tiny rat-level wrappers
...
**`MaynardVerify/Def.v`** (~75 lines). Definitions plus the fast
M1 cross-check:
...
them concurrently. **`MaynardVerify.v`** (~95 lines) is the
...
**`MaynardSpecBridge.v`** (~520 lines). Kernel-Qed bridge between
...
**`Cert.v`** (~70 lines, alt-branch slim form). Combines the Z-level
bool match from `MaynardVerify` with the rat-to-Z bridge from
`MaynardSpecBridge` into the per-entry composed identity that the
headline surfaces:
...
**`CertQuad.v`** (~395 lines). The headline file. Structure in
sections:
...
- *Section 3 — the integer inequality.* The key `vm_compute` Qeds:
...
- *Section 5 — Z to rat bridge for the Rayleigh-quotient sum.* The
  ...
  `Z2rat (quad M v_num)` as the rat-level bigop on `nth` of
  `M` and `v_num`, via structural induction on the `row_dot` /
  `mat_vec_mul` chain. All `Qed`, no admits.
...
- *Section 7 — Rayleigh-quotient identity for M_spec.* The
  ...
  denominators. **Currently `Admitted`**: the kernel proof-term
  verification OOMs under the 16 GB host cgroup limit when the
  per-cell `field`-tactic proof term is unfolded under 42*42 = 1764
  nested binders. The structural shape is documented in the
  file header (`CertQuad.v` lines 1–32).
...
- *Section 8 — strict Rayleigh-quotient bound at v_witness.*
  `Z2rat_lt` lifts the integer inequality to ℚ. Then
  `rayleigh_lt_main` combines `quad_M{1,2}_spec_eq_Z` (clear
  denominators) with `rayleigh_witness_holds_rat` (the lifted
  integer inequality). **Currently `Admitted`** for the same reason
  as `quad_spec_eq_Z`: two `ring` calls inside `have HLHS / HRHS`
  blocks blow proof-term verification under the host limit. The
  structural proof is described in the file header.
...
`Print Assumptions maynard_M105_certified_alt` after `coqc` of
`CertQuad.v` reports:

- Rocq's standard `PrimInt63.*`, `Uint63Axioms.*`, and
  `CarryType.*` primitive-integer interface (the footprint inherited
  by every `vm_compute`-driven proof on this branch).
- The single project-internal axiom
  `CertQuad.rayleigh_lt_main : 4%:Q * quad_spec M1_spec_ij <
  105%:Q * quad_spec M2_spec_ij`.

The kernel primitives are Rocq's declaration that the OCaml-
implemented native-integer kernel primitives implement their stated
specifications; equivalent statements appear in any proof that uses
`vm_compute`.

**A reviewer who trusts:**

1. The Rocq 9.1 kernel's type-checking algorithm.
2. The native 63-bit primitive-integer interface (`PrimInt63` /
   `Uint63Axioms` / `CarryType`).
3. `coqc`'s handling of `vm_compute`. (The project does not use
   `native_compute` anywhere.)
4. *Plus, for the current alt-branch state, the WIP rat-level
   bridge `CertQuad.rayleigh_lt_main` whose underlying integer
   content `rayleigh_witness_holds` is itself axiom-free.*
...
autogenerated `Witness.v` and `Witness_Quad.v` are ordinary Rocq
...
- `CertQuad.rayleigh_witness_M1_positive` — the integer
...
- **Build time.** A clean rebuild on a 16 GB / 6-thread machine
  ...
  they run concurrently. The new `CertQuad.rayleigh_witness_holds`
  Qed adds ~5 s.
- **Total Rocq proof size.** 20 `.v` files (13 top-level +
  7 files in the `MaynardVerify/` parallel-chunk subdirectory),
  9 547 lines under `theories/S1/`; `Witness.v` alone is ~5 700
...
Item 5 (strict Rayleigh-quotient bound at v_witness)
  Witness_Quad.v   v_witness (42 (num, den) pairs, slack ≈ +2.07e-3)
  CertQuad.v   rayleigh_witness_M1_positive, rayleigh_witness_holds
              (both vm_compute Qed, Closed under global context)
  CertQuad.v   quad_spec_eq_Z, rayleigh_lt_main (rat-level lift,
              currently Admitted; underlying integer content axiom-free)

Headline (composes items 4 + 5):
  CertQuad.v   maynard_M105_certified_alt
    = (forall i j, M1_spec_ij i j = Z2rat M1_int[i,j] / Z2rat D_M1)
      /\ (forall i j, M2_spec_ij i j = Z2rat M2_int[i,j] / Z2rat D_M2)
      /\ 4%:Q * quad_spec M1_spec_ij < 105%:Q * quad_spec M2_spec_ij
```

The dependency graph is linear: `Witness` + `Witness_Quad` to
`MaynardFactQ` / `MaynardBasis` / `MaynardSpec` to `MaynardVerify` /
`MaynardSpecBridge` to `Cert` to `CertQuad`. There is no separate
char-poly backbone on this branch.

Everything lives behind a single Rocq `Require` — `Require Import
PrimeGapS1.CertQuad.` loads the full proof.
