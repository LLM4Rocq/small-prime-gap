...
\rocq{PrimeGapS1.CertQuad.rayleigh_lift_generic}
\rocqok
...
\texttt{Print Assumptions maynard\_M105\_certified\_alt} reports a
single line: \emph{Closed under the global context}. This is
stronger than the typical \vmcompute{} footprint: because the
matrices and the witness vector are encoded as \texttt{list (list
Z)} and \texttt{list (Z * Z)} rather than as native 63-bit packed
arrays, \vmcompute{} reduces through $\Z$ arithmetic instead of
through the $\Uintsf$ kernel primitives. None of
\texttt{PrimInt63}, \texttt{Uint63Axioms}, or \texttt{CarryType}
appear in the reported assumptions.
...
\texttt{MaynardVerify/} parallel-chunk subdirectory), ${\sim}9{,}590$
