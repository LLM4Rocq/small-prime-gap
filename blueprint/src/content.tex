...
The headline Rocq result states this in Maynard's own spectral
language: it exhibits, inside the kernel, an eigenvalue $> 4$ of the
rational matrix $M_{105} = 105 \cdot \Mone^{-1}\Mtwo$ over $\algC$
(MathComp's algebraic complex numbers), matching Maynard's
characterisation of $\Mk$ as ($k$ times) the largest eigenvalue of the
generalised eigenproblem $\Mtwo v = \lambda \Mone v$.

The \emph{computational heart} of that result is a single integer
Rayleigh inequality. Following Maynard's notebook --- instead of
computing $\lammax(\Mone^{-1}\Mtwo)$ numerically --- we exhibit one
42-entry rational \emph{witness vector} $v_{\mathrm{witness}} \in
\rat^{42}$ (shipped in \texttt{Witness\_Rayleigh.v}, obtained by
snapping the top eigenvector of $\Mone^{-1}\Mtwo$ to small-denominator
rationals via continued-fraction convergents) and verify the strict
...
\vmcompute{} reflexivity --- the kernel-checked counterpart of the
notebook's numerical evaluation. Turning that quotient bound into a
positive \emph{eigenvalue} requires two further ingredients: that
$\Mone$ is positive definite (a deterministic CRT-over-$\Z$
characteristic-polynomial sign certificate) and a Hermitian spectral
bridge supplying a positive eigenvalue from a strictly positive
...
\end{verbatim}
proven in \texttt{theories/S1/MaynardEigen.v}, where $M_{105} = 105
\cdot \Mone^{-1}\Mtwo$ as a MathComp matrix over $\rat$, \texttt{ratrM}
is the entrywise $\rat \to \algC$ embedding, and
\texttt{matches\_closed\_forms} certifies that $M_{105}$ is built from
the FLINT-shipped integer matrices, each of which matches Maynard's
closed forms (Lemma~8.2) entrywise. The first conjunct anchors the
input data to the paper; the second is the spectral conclusion
$\Mk[105] > 4$.
...
\paragraph{Status.}
The headline theorem and every lemma it depends on is \Qed{} in the
source tree, with no project-specific axioms (the reported assumptions
are discussed in Chapter~\ref{ch:trust}).

\paragraph{The notebook's own conclusion, standalone.}
The same integer Rayleigh inequality is also packaged on its own as
\rocq{PrimeGapS1.CertRayleigh.maynard_M105_certified_rayleigh}: $\Mk[105]
> 4$ read directly off the Rayleigh quotient (via Maynard's Lemma~8.3),
without the eigenvalue reformulation. This is the notebook's conclusion
in its original shape; it uses neither the spectral theorem nor the CRT
positive-definiteness certificate, depending only on the matrix
transcription and the one integer \vmcompute{}. The headline
\rocq{PrimeGapS1.MaynardEigen.maynard_M105_certified} goes one step
further and recasts the bound as an eigenvalue.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{The Rayleigh inequality: the notebook computation, mechanised}\label{ch:rayleigh}

This chapter is the mechanised form of Maynard's notebook computation.
The notebook snaps the top eigenvector of $\Mone^{-1}\Mtwo$ to a
small-denominator rational vector and evaluates the matrix's Rayleigh
quotient there in exact rational arithmetic; we replay exactly that
evaluation in the kernel. It is a single strict inequality that, at one
witness vector, both certifies $\Mk[105] > 4$ directly (via Maynard's
Lemma~8.3) and supplies the positivity input the spectral bridge of
Chapter~\ref{ch:eigen} consumes. Maynard's Lemma~8.3 reduces
$\Mk[k] > c$ to: exhibit a single coefficient vector
$a \in \rat^{42}$ with $k \cdot a^T \Mtwo a / a^T \Mone a > c$,
equivalently $c \cdot a^T \Mone a < k \cdot a^T \Mtwo a$ (and
$a^T \Mone a > 0$ so the quotient is well-defined). The shipped
rational vector $v_{\mathrm{witness}}$ in \texttt{Witness\_Rayleigh.v}
is such an $a$ for $(k, c) = (105, 4)$. The kernel verifies the strict
integer inequality at $v_{\mathrm{witness}}$ by a single \vmcompute{}
reflexivity on $\Z$-arithmetic --- this is the step that stands in for
the notebook's numerical evaluation; the supplementary algebraic shim
lifts that integer fact to the rat-level bigop
(Theorem~\ref{thm:rayleigh_lt_main}) the eigenvalue route consumes.

The material lives in \texttt{Witness\_Rayleigh.v} (the witness vector
itself) and \texttt{CertRayleigh.v} (the integer Rayleigh inequality,
its positivity companion, the rat-level lift to the bigop on the
paper-form spec matrices, and the standalone theorem
\texttt{maynard\_M105\_certified\_rayleigh}, the notebook's own
conclusion read off the Rayleigh quotient).
...
\rocq{PrimeGapS1.CertRayleigh.rayleigh_witness_M1_gt0}
\rocqok
\uses{def:num_M1M2}
...
the comparison itself is one BigZ subtraction.
\end{proof}
...
restricted to $[LHS]$ to keep unification cheap.
\end{proof}
...
lifted to $\rat$ by $\texttt{Z2rat\_lt}$.
\end{proof}

\section{The standalone theorem: the notebook's conclusion}
...
\paragraph{Reading.}
Theorem~\ref{thm:M105} is the notebook's own conclusion, stated on its
own: a certified strict Rayleigh-quotient bound on the paper-form spec
matrices at the shipped 42-entry rational witness vector, together with
the closed-form match of those matrices to Maynard's specification.
Modulo Maynard's Lemma~8.3 (an Annals-published Rayleigh-quotient
identity, paper-side) it gives $\Mk[105] > 4$. It has no eigenvalue, no
characteristic polynomial, and no CRT lift in its dependency tree ---
the Rayleigh quotient is all the notebook needed. The headline theorem
of Chapter~\ref{ch:eigen} reaches the same conclusion in Maynard's own
spectral form, built on top of this same inequality.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{The eigenvalue certification (the headline)}\label{ch:eigen}

This chapter establishes the headline result. It states $\Mk[105] > 4$
in Maynard's own \emph{characterisation} of $\Mk$ as ($k$ times) the
largest eigenvalue of the generalised eigenproblem
$\Mtwo v = \lambda \Mone v$: it exhibits, inside the kernel, an
eigenvalue $> 4$ of the rational matrix
$M_{105} = 105 \cdot \Mone^{-1}\Mtwo$. The headline is
\rocq{PrimeGapS1.MaynardEigen.maynard_M105_certified}
(Theorem~\ref{thm:M105_eigen}).

It is built \emph{on} the computational core of
Chapter~\ref{ch:rayleigh}: the strict integer Rayleigh inequality of
Theorem~\ref{thm:rayleigh_lt_main} is exactly the positivity input the
spectral bridge consumes. In contrast to the Rayleigh route, this
development genuinely \emph{does} build a characteristic
polynomial (modulo primes), perform a Chinese-remainder lift, and
exhibit an eigenvalue, all over Stdlib's $\Z$ and $\Fp$ with no
\texttt{native\_compute} and no real-algebra (IVT / Sturm / $\realalg$)
machinery.
...
\section{Positive definiteness of \texorpdfstring{$\Mone$}{M\_1}}
...
The \texttt{Some} branch factors through two structural lemmas:
...
\begin{theorem}[End-to-end eigenvalue certification (the headline)]\label{thm:M105_eigen}
\rocq{PrimeGapS1.MaynardEigen.maynard_M105_certified}
\rocqok
...
Lemma~\ref{lem:eigen_bridge}.
\end{proof}

\paragraph{Reading.}
This is the headline result, stated in Maynard's own spectral language:
$M_{105}$ has an eigenvalue exceeding $4$. Its only numerical input
beyond the matrix data is the strict Rayleigh inequality of
Chapter~\ref{ch:rayleigh} (Theorem~\ref{thm:rayleigh_lt_main}); the same
inequality, packaged standalone as Theorem~\ref{thm:M105}, gives the
notebook's conclusion $\Mk[105] > 4$ directly, without the eigenvalue
reformulation.
...
\item Every claim of the form $a = b$ in $\Z$ or $\Q$, $a < b$ in
  ...
  appears in the dependency tree of the headline
  Theorem~\ref{thm:M105_eigen} (or of the standalone
  Theorem~\ref{thm:M105}) is \Qed{} in the source tree.
...
\texttt{Print Assumptions} on the headline
\rocq{PrimeGapS1.MaynardEigen.maynard_M105_certified} and on the
standalone \rocq{PrimeGapS1.CertRayleigh.maynard_M105_certified_rayleigh}
reports \emph{Closed under the global context}: no project-specific axioms.
...
\texttt{CertRayleigh.v}, producing the computational-core inequality
Theorem~\ref{thm:rayleigh_lt_main} and the standalone
Theorem~\ref{thm:M105} (\texttt{Require Import
PrimeGapS1.CertRayleigh.} loads them); the eigenvalue backbone builds on
that inequality to produce the headline Theorem~\ref{thm:M105_eigen}
