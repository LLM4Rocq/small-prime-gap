...
\vmcompute{} reflexivity. Turning that quotient bound into a positive
\emph{eigenvalue} requires two further axiom-free ingredients: that
...
\paragraph{Minimal-trust anchor.}
The same integer Rayleigh inequality is also packaged standalone as
\rocq{PrimeGapS1.CertRayleigh.maynard_M105_certified_rayleigh}: a proof
of $\Mk[105] > 4$ (via Maynard's Lemma~8.3) whose only kernel
obligations are the matrix transcription and one integer \vmcompute{},
needing neither the spectral theorem nor the CRT positive-definiteness
certificate. It is a minimal-trust audit anchor for a reviewer who
wants the inequality without the eigenvalue machinery. Both it and the
headline \rocq{PrimeGapS1.MaynardEigen.maynard_M105_certified} are
\Qed{} and report \emph{Closed under the global context}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{The computational core: the integer Rayleigh inequality}\label{ch:rayleigh}

This chapter is the computational core of the headline eigenvalue
result (Chapter~\ref{ch:eigen}): the single strict inequality that, at
one witness vector, both certifies $\Mk[105] > 4$ directly (via
Maynard's Lemma~8.3, the minimal-trust anchor) and supplies the
positivity input the spectral bridge consumes. Maynard's Lemma~8.3
reduces $\Mk[k] > c$ to: exhibit a single coefficient vector
...
reflexivity on $\Z$-arithmetic; the supplementary algebraic shim lifts
that integer fact to the rat-level bigop
...
paper-form spec matrices, and the standalone minimal-trust theorem
\texttt{maynard\_M105\_certified\_rayleigh}).
...
\section{The standalone minimal-trust theorem}
...
\paragraph{Reading.}
Theorem~\ref{thm:M105} is the \emph{minimal-trust anchor}: a reviewer
who trusts \emph{only Rocq's kernel} obtains a certified strict
Rayleigh-quotient bound on the paper-form spec matrices at the shipped
42-entry rational witness vector, together with the closed-form match
of those matrices to Maynard's specification --- with no eigenvalue, no
characteristic polynomial, and no CRT lift in its dependency tree. A
reviewer who additionally accepts Maynard's Lemma~8.3 (an
Annals-published Rayleigh-quotient identity, paper-side) obtains
$\Mk[105] > 4$ end-to-end. The headline theorem of
Chapter~\ref{ch:eigen} reaches the same conclusion in Maynard's own
...
spectral bridge consumes. Everything below is \Qed{} and axiom-free; in
particular this development genuinely \emph{does} build a characteristic
...
beyond the matrix data is the strict Rayleigh inequality of the
computational core (Theorem~\ref{thm:rayleigh_lt_main}); the same
inequality, packaged standalone as Theorem~\ref{thm:M105}, is the
minimal-trust anchor that reaches $\Mk[105] > 4$ without the eigenvalue
machinery.
...
\item Every claim of the form $a = b$ in $\Z$ or $\Q$, $a < b$ in
  ...
  Theorem~\ref{thm:M105_eigen} (or of the minimal-trust anchor
  Theorem~\ref{thm:M105}) is
...
\rocq{PrimeGapS1.MaynardEigen.maynard_M105_certified}
(Theorem~\ref{thm:M105_eigen}) and its standalone minimal-trust anchor
\rocq{PrimeGapS1.CertRayleigh.maynard_M105_certified_rayleigh}
(Theorem~\ref{thm:M105}) \emph{both} report a single line under
\texttt{Print Assumptions} — \emph{Closed under the global context}.
This is stronger than the typical \vmcompute{} footprint: because the
matrices and the witness vector are encoded as \texttt{list (list Z)}
and \texttt{list (Z * Z)} rather than as native 63-bit packed arrays,
\vmcompute{} reduces through $\Z$ arithmetic — and, for the CRT
positive-definiteness certificate behind the headline, through
$\Fp = \Z/p\Z$ — instead of through the $\Uintsf$ kernel primitives.
None of \texttt{PrimInt63}, \texttt{Uint63Axioms}, or \texttt{CarryType}
appears in the reported assumptions of either theorem.
...
Theorem~\ref{thm:rayleigh_lt_main} and the standalone minimal-trust
anchor Theorem~\ref{thm:M105} (\texttt{Require Import
