...
(*  Performance test                                                *)
(*                                                                  *)
(*  Build a 42-degree polynomial with 200-bit coefficients and      *)
(*  compute pmul and prem on it. Must finish under a second.        *)
(* ============================================================== *)

(* A ~200-bit coefficient. *)
Definition big_coef : Z :=
  1606938044258990275541962092341162602522202993782792835301376.
(* = 2^200 *)

(* Build a coefficient list of given length with large coefficients.
   Element k gets value `big_coef + shift + k`. *)
Fixpoint big_list (n : nat) (shift : Z) : list Z :=
  match n with
  | O => []
  | S k => (big_coef + shift) :: big_list k (shift + 1)
  end.

(* Degree 42 polynomial (43 coefficients) with 200-bit entries. *)
Definition big_p : pol := big_list 43 0.

(* Degree 20 divisor with 200-bit entries. *)
Definition big_q : pol := big_list 21 100.

(* Sanity: the pseudo-remainder of big_p by big_q has degree less than
   deg big_q = 20, so its size is at most 20.  We assert equality of
   psize to a concrete value after vm_compute. *)
Example perf_prem_degree_bound :
  Nat.ltb (psize (prem big_p big_q)) 21 = true.
Proof. vm_compute. reflexivity. Qed.

(* And a pmul performance test: multiply big_p (degree 42) by
   [1; 0; 1] (degree 2) and check its degree is 44. *)
Example perf_pmul_degree :
  psize (pmul big_p [1; 0; 1]) = 45%nat.
Proof. vm_compute. reflexivity. Qed.
