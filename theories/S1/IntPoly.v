...
(* # IntPoly                                                                  *)
(*                                                                            *)
(* A plain `list Z` polynomial library for the computational layer of the     *)
(* Maynard S1 proof.                                                          *)
(*                                                                            *)
(* MathComp's `{poly R}` does not reduce under `vm_compute` at the sizes we   *)
(* need (42-degree polys, 200-bit coefs). Stdlib `ZArith` + plain `list Z`    *)
(* does, so the whole computational workload lives here.                      *)
(*                                                                            *)
(* ```                                                                        *)
(* pol == a polynomial as its LOW-TO-HIGH coefficient list                    *)
(* ```                                                                        *)
(*                                                                            *)
