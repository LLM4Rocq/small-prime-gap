(* CRTLift.v — CRT-side infrastructure for quad's pencil chain.

   Phase B slim: dropped the 43-coefficient fl_eq_flint chain and the
   matrix-identity-Z chain that previously lifted per-prime checks back
   to Z.  The quad chain works on rat-level A_rat and reads only
   det_M1_int / D_pencil_int as integer-side inputs.
